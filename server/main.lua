...
ESX.RegisterServerCallback('esx_shopsadd:requestDBItems', function(source, cb)

	MySQL.Async.fetchAll(
		'SELECT * FROM shopsadd',
		{},
		function(result)
			local shopItems  = {}
			for i=1, #result, 1 do
...
RegisterServerEvent('esx_shopsadd:buyItem')
AddEventHandler('esx_shopsadd:buyItem', function(itemName, price)
